...
export const QuizesSkeleton = ({ size, isScrollable }: { size: number, isScrollable: boolean }) => {
  return (
    <div className={cn("w-full flex flex-wrap justify-center items-start gap-8 max-sm:gap-5", isScrollable && 'flex-nowrap')}>
      {Array.from({ length: size }).map((_, idx) => {
        return (
          <Card key={idx} className="w-[250px] h-[412px] max-sm:w-full space-y-5 p-4 shrink-0" radius="lg">
